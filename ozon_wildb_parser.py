...
class Parser(abc.ABC):
    ...
    @logger.catch
    @staticmethod
    def writing_in_csv(list_of_dict: list[dict], name: str, fieldnames: list, encoding="utf-8", newline="",
                       delimiter=";"):
        print(f"Идёт запись в файл {name}")
        with open(f"{name}.csv", "a", encoding=encoding,
                  newline=newline) as csv_file:  # Определили список заголовков столбцов
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames,
                                    delimiter=delimiter)  # Подготовили дескриптор файла для записи CSV
...
class ParserWildberries(Parser):
    ...
    def _parse_data(self, source, timing=2):
        ...
        date_html = soup.find_all("span", class_="feedback__date hide-mobile")
        for i in date_html:
            try:
                date.append(datetime.strptime(i.get('content'), '%Y-%m-%dT%H:%M:%SZ') + timedelta(hours=3))
            except Exception as e:
                date.append(datetime.strptime(i.get('content'), '%Y-%m-%dT%H:%M:%S.%fZ') + timedelta(hours=3))
        ...
        for review in zip(date, stars, reviews, likes, dislikes):
    ...
    def get_data(self):
        ...
        super().get_data(link=self.link, timing=3)
        # region SCROLL DOWN
        ...
        try:
            ...
            scroll_iterations = 101
        print(scroll_iterations)
...
class ParserOzon(Parser):
    ...
    def __init__(self, link):
        ...
        self.fieldnames = ['ID', 'Количество звезд', 'Дата', 'Достоинства', 'Недостатки', 'Комментарии', 'Лайки',
                           'Дизлайки']
        ...
        for review in reviews_webelements:
            ...
            if "Достоинства" in review.text:
                if "Недостатки" not in review.text:
                    single_review["Достоинства"] = re.compile(
            ...
            elif "Комментарии" in review.text:
    ...
    def get_data(self, link='', timing=7):
        ...
        while True:
            ...
            time.sleep(timing)
            # links = super().browser.find_elements(By.CSS_SELECTOR, 'span.rl2')
            # for _ in range(3):
            #     time.sleep(0.3)
            #     links[randint(0, len(links) - 1)].click()
            try:
...
if __name__ == "__main__":
    links_ozon = [
        'https://www.ozon.ru/product/nabor-posudy-dlya-prigotovleniya-rondell-6-predm-552179043'
    ]

    links_wild = [
        'https://www.wildberries.ru/catalog/2025362/detail.aspx'
    ]
